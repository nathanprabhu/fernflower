...
public class ControlFlowGraph implements CodeConstants {
    ...
    private  void connectBlocks(List<BasicBlock> lstbb, Map<Integer, BasicBlock> mapInstrBlocks) {
         System.out.println("lstbb Size:"+lstbb.size());
        //	  ExecutorService executor = Executors.newCachedThreadPool();
        for (int i = 0; i < lstbb.size(); i++) {
            
            System.out.println("lstbb Size inside loop:"+lstbb.size());
            System.out.println("Sending thread:"+i+"to worker thread");
            
            Runnable worker=new WorkerThread1(i,lstbb,mapInstrBlocks);
